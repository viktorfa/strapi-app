...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Offer
      .findOne(_.pick(params, _.keys(Offer.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count offers.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Offer.create(data);

    // Create relational data and return the entry.
    return Offer.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Offer.associations.map(a => a.alias));
    const data = _.omit(values, Offer.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Offer.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Offer.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Offer.attributes).reduce((acc, curr) => {
      switch (Offer.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
