...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Organization
      .findOne(_.pick(params, _.keys(Organization.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count organizations.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Organization.create(data);

    // Create relational data and return the entry.
    return Organization.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Organization.associations.map(a => a.alias));
    const data = _.omit(values, Organization.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Organization.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Organization.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Organization.attributes).reduce((acc, curr) => {
      switch (Organization.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
